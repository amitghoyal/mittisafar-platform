...
// Fetch experiences for the logged-in host
$query = "SELECT experience_id, title, category, price, image FROM experiences WHERE host_id = ? ORDER BY created_at DESC";
...
<main class="main">
    <div class="container mt-5">
        <h2 class="text-center mb-4">My Experiences</h2>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card shadow-lg">
                            ...
                            <div class="carousel-inner">
                                <?php foreach ($images as $index => $img): ?>
                                    <div class="carousel-item <?= $index === 0 ? 'active' : ''; ?>">
                                        <img src="../uploads/<?= htmlspecialchars($img); ?>" class="d-block w-100" alt="<?= htmlspecialchars($row['title']); ?>">
                                    </div>
                                <?php endforeach; ?>
                            </div>
                            <!-- Controls -->
                        ...
                        <div class="card-body text-center">
                            ...
                            <p class="text-muted"><strong>Category:</strong> <?= htmlspecialchars($row['category']); ?></p>
                            <p class="text-dark"><strong>Price:</strong> ₹<?= htmlspecialchars($row['price']); ?></p>
                            <button class="btn btn-view text-white view-details" data-id="<?= $row['experience_id']; ?>">View Details</button>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
...
$(document).ready(function() {
    $(".view-details").click(function() {
        ...
        $.ajax({
            url: "fetch_experience_details.php",
            type: "POST",
